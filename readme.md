...
1. Run using the --download option. Usually the best way and recommended to do periodically anyway (perhaps annually or more often depending on your use case).  
2. Run a brute force change detection process between a clean copy and your target feature class. You might choose to do this if your target is versioned and a full delete/append is not desirable. There are Python scripts and FME tools that can do this sort of comparison.
...
> LINZ occasionally change the schema of datasets. In the UI of the LINZ website, this is sometimes listed in the history tab. What could be done to detect and warn if the schema had changed? Is it worth attempting to query the LINZ API history? Or just do a comparison of the schema of the data once it is fetched? In either case, if a change in schema is detected, what mechanism could be used to alert? Should sending notifications be the responsibility of this module, or should this module just raise an error that someone implementing it be responsible for catching this (and/or any other error messages) and implementing some notification? Probably up to the user, as the notification process might differ per implementation. Email, Teams, Slack, etc.
> How can the update part of applying edits be made faster? The current approach of using a search cursor and update cursor is very slow.